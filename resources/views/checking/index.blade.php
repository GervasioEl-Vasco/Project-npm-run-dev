                ...
                <form action="{{ route('checking.store', $order->id) }}" method="POST">
                    ...
                    <div class="mb-6 p-4 rounded-xl border" :class="adaTemuan ? 'bg-yellow-50 border-yellow-200' : 'bg-gray-50 border-gray-200'">
                        <div class="flex items-center justify-between">
                            <span class="text-sm font-bold text-gray-700">Apakah ada barang tertinggal di saku?</span>
                            <button type="button" @click="adaTemuan = !adaTemuan" 
                                ...
                                <span x-text="adaTemuan ? 'Ada Temuan' : 'Tidak Ada'">Tidak Ada</span>
                            </button>
                        </div>
                        
                        <div x-show="adaTemuan" x-transition class="mt-4">
                            <label for="items_found" class="block text-xs font-bold text-gray-600 uppercase mb-1">Daftar Barang Temuan (misal: Uang Rp50.000, Kunci)</label>
                            <textarea name="items_found" id="items_found" x-model="catatanTemuan" rows="3"
                                      class="w-full rounded-lg border-gray-300 focus:ring-indigo-500 focus:border-indigo-500" 
                                      placeholder="Tulis detail barang temuan agar tercatat di struk nota..."></textarea>
                        </div>
                    </div>
